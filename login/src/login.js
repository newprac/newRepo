...
app.post("/register", async (req, res) => {
  ...
  const checkUsernameQuery =
    "SELECT COUNT(*) AS count FROM your_database WHERE id = ?";
  db.query(checkUsernameQuery, [id], async (err, results) => {
    ...
    const checkNicknameQuery =
      "SELECT COUNT(*) AS count FROM your_database WHERE nickname = ?";
    db.query(checkNicknameQuery, [nickname], async (err, results) => {
      ...
      try {
        ...
        const insertUserQuery =
          "INSERT INTO your_database (id, password, nickname, email, birthday) VALUES (?, ?, ?, ?, ?)";

        db.query(
          insertUserQuery,
          [id, hashedPassword, nickname, formattedEmail, formattedBirthday],
          (err, result) => {
            if (err) {
              return res.status(500).json({ error: err.message });
            }
...
app.get("/user-info", (req, res) => {
  ...
  const findUserQuery =
    "SELECT email, birthday FROM your_database WHERE id = ?";
...
app.put("/update-user", (req, res) => {
  ...
  db.query(
    updateUserQuery,
    [email || null, birthday || null, nickname || null, id],
    (err, result) => {
      if (err) {
        return res.status(500).json({ error: "서버 오류가 발생했습니다." });
      }
  ...
  db.query(findUserQuery, [id], async (err, results) => {
    ...
    try {
      ...
      const updatePasswordQuery =
        "UPDATE your_database SET password = ? WHERE id = ?";
      ...
      res
        .status(500)
        .json({ error: "비밀번호 암호화 중 오류가 발생했습니다." });
    }
  });
});
...
app.post("/update-profile", (req, res) => {
  ...
  const query =
    "UPDATE your_database SET nickname = ?, email = ?, birthday = ? WHERE id = ?";
